...
\section{Test Cases}
    ...
    \textbf{Coverage on Desktop/Laptop App} 
   	Since there are unit tests that will not be tested on the mobile and web apps because they have less functionality than the desktop/laptop app, statement coverage and branch coverage will be described for the desktop/laptop platform. 
   	\begin{itemize}
   		\item \textbf{Statement Coverage:} The test cases mentioned for the unit test of all the classes to be tested will need to use and test all of their respective methods; hence yielding a 100\% statement coverage. 
   		\item \textbf{Branch Coverage:} For the sake of simplicity, for any method that returns a String, int, or float, 100\% branch coverage means that it either successfully returns the type or throws an exception. 100\% branch coverage for methods that return a boolean value means that it either returns true or false.
   		\begin{itemize}
	   		\item \textbf{Course:} As for methods that return the type Job, the plan is to test for one set of inputs that successfully addJob and for two sets of inputs that fail to do so. The plan is to have 75\% branch coverage for the Course class. 
	   		\item \textbf{Job:} The plan is to test all methods for their successful return. The Job class has far too many methods to test for 100\% branch coverage. The plan is to test for the correct set of inputs for each method and for the incorrect set of inputs for specific methods that could cause issues. This yields a branch coverage plan of upwards of 50\%, (40\% for the correct input for each method, and 10\% for the incorrect input for specific methods mentioned in the Test Cases). Laboratory and Tutorial will have the same plan and coverage as Job since they inherit from the Job class.
	   		\item \textbf{Profile:} The plan is to test the correct input for all methods and sets of incorrect inputs that will (null, empty, not long enough passwords) cause issues. The plan is to yield a branch coverage of 75\% and more. Student, Instructor, and Admin will follow the same plan as Profile since they inherit from the Profile class.
	   		\item \textbf{Application:} The plan is to test sets of correct inputs for each method, and multiple sets of incorrect inputs that will cause issues. Given the small amount of methods to test for this class, the plan is to yield a branch coverage of 75\% and more.
   		\end{itemize} 
   	\end{itemize}
    Since the plan is to implement certain functionalities in a Test Driven Development paradigm, it will be necessary to execute tests before there is even code written for the test. Tests will be executed often during the development of new functionality and during the verification and validation of existing functionality. In other words, JUnit tests will be written and executed before a certain method is written. JUnit tests will be written and executed shortly after test cases are made for a certain method that is already implemented. 
    
    The desktop/laptop app has all functionality; hence, all unit tests for all of the classes to be tested will be at least done on this platform. The mobile and web apps have less functionality than the desktop/laptop app; hence, there will be less classes to be tested on these two platforms. 
    In this situation, the mobile app is implemented for the student's use; hence, there will be no unit testing for the Admin, Instructor, and Course classes on this platform. More specifically, the plan of unit testing the mobile app is to unit test the Student, Application, and Job classes. 
    The web app is implemented for the instructor's use; hence, there will be no unit testing for the Admin, and Student classes on this platform. More specifically, the plan of unit testing the web app is to unit test the Instructor, Application, Job, and Course classes.
...
From the thoroguh enumeration above, it is clear that all use cases of the system (as well as all of
